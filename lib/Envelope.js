...
Envelope.prototype.open = function open(keypair){ console.log()

  const payloadOffset = this.data[0] === 0 ? 1 : util.SIGNATURE_LENGTH
  ...
  if (message.flags === util.SIGNATURE_FLAG) {
    message.signature = data.splice(0, util.SIGNATURE_LENGTH)
  }

  message.decrypt(keypair)
